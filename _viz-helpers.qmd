...
```{ojs}
//| output: false
// group_size and option_size are ordered numeric variables with an open-ended
// set of levels (new group/option sizes show up as datasets are added — e.g.
// option_size gained 18 and 36 after this was first written) — a sequential
...
// the observed x-range of those rows. No-ops if weightedLogTrend can't fit a line.
...
function addTrend(marks, rows, { x, y, weight }, color, withCI, facet) {
  const predict = weightedLogTrend(rows, { x, y, weight })
  if (!predict) return
  ...
  const predicted = grid.map(xVal => ({ x: xVal, ...predict(xVal), ...facet }))
...
function linePlot(rows, { y, title, yLabel, ylim = null, groupingVar, facetVar, indivLines, stageOneOnly, containerWidth, groupLabel, facetLabel }) {
  ...
  if (indivLines) {
    marks.push(Plot.line(rows, { x: xVar, y: yValue, z: "game_id", stroke: colorChannel, strokeOpacity: 0.05, ...facetChannels }))
  }
  marks.push(Plot.dot(rows, { x: xVar, y: yValue, fill: colorChannel, fillOpacity: 0.3, stroke: null, r: 2.5, ...facetChannels }))
  ...
  // each other.
  const facetTargets = isFaceted ? d3.groups(rows, d => d[facetVar]) : [[null, rows]]
  for (const [facetKey, facetRows] of facetTargets) {
    ...
    if (isDefaultGrouping) {
      ...
      for (const trendRows of trendGroups) {
        addTrend(marks, trendRows, { x: xVar, y, weight }, "black", true, facetTag)
      }
    } else {
      for (const [level, levelRows] of d3.groups(facetRows, d => d[groupingVar])) {
        ...
        for (const trendRows of trendGroups) {
          addTrend(marks, trendRows, { x: xVar, y, weight }, color, false, facetTag)
        }
      }
    }
  }
  ...
  const plot = Plot.plot({
    ...
    y: { label: yLabel, domain: ylim ?? undefined },
  ...
  const xCaption = html`<div style="text-align: right; font-size: 1rem; margin-top: 2px; width: ${xCaptionWidth}px;">Repetition →</div>`
...
// dots + trend lines).
function stackPlot(rows, { y, title, yLabel, groupingVar, facetVar, containerWidth, groupLabel, facetLabel }) {
  const xVar = "rep_num"
  const isFaceted = facetVar !== "dataset_id"
  const spec = colorSpec(rows, groupingVar)
