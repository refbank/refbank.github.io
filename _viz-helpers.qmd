...
// choice array (round numbers, or the group/option-size values present in
...
// and response time that reduce proportionally across rounds rather than
...
function linePlot(rows, { y, title, yLabel, ylim = null, groupingVar, facetVar, indivLines, stageOneOnly, containerWidth, groupLabel, facetLabel, logX = false, logY = false, logLogTrend = false }) {
  ...
  // Ticks beyond the data's actual x-extent (e.g. the Round filter
  ...
  const xCaption = html`<div style="text-align: right; font-size: 1rem; margin-top: 2px; width: ${xCaptionWidth}px;">Round${logX ? " (log scale)" : ""} →</div>`
...
// round, stacked by `groupingVar` (always POS type on the PoS tab — this
...
function stackPlot(rows, { y, title, yLabel, groupingVar, facetVar, containerWidth, groupLabel, facetLabel, logX = false }) {
  ...
  // Aggregate to mean(y) per round × facet level × grouping level —
